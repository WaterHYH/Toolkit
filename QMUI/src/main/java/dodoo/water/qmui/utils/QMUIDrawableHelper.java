...
package dodoo.water.qmui.utils;
...
public class QMUIDrawableHelper {
    ...
    /**
     * @see QMUIDrawableHelper#createBitmapFromView(View, float)
     * @param view
     * @return
     */
    ...
    /**
     * 从一个view创建Bitmap。把view的区域截掉leftCrop/topCrop/rightCrop/bottomCrop个像素
     * 注意：裁剪长宽必须大于零，否则返回null
     */
    public static Bitmap createBitmapFromView(View view, int leftCrop, int topCrop, int rightCrop, int bottomCrop) {
        int width = view.getWidth() - rightCrop - leftCrop;
        int height = view.getHeight() - topCrop - bottomCrop;
        if (width <= 0 || height <= 0) {
            return null;
        }
        ...
        Bitmap cutBitmap = createBitmapSafely(width, height, Bitmap.Config.ARGB_8888, 1);
        if (cutBitmap == null) {
            return null;
        }
